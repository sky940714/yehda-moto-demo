...
export async function PATCH(request:Request){if(!await admin())return NextResponse.json({error:"無管理權限。"},{status:403});try{const body=await request.json() as {id?:unknown;status?:unknown;note?:unknown;refundMethod?:unknown;refundAmount?:unknown;refundReference?:unknown};await updateReturnStatus(Number(body.id),String(body.status),String(body.note||""),String(body.refundMethod||""),Number(body.refundAmount||0),String(body.refundReference||""));return NextResponse.json({ok:true});}catch(error){return NextResponse.json({error:error instanceof Error?error.message:"更新失敗。"},{status:400});}}
