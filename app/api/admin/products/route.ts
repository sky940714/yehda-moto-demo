...
export async function PATCH(request:Request){const user=await admin();if(!user)return NextResponse.json({error:"無管理權限。"},{status:403});const body=(await request.json()) as {ids?:unknown;status?:unknown},ids=normalizeIds(body.ids),status=String(body.status||"");if(!["active","draft","out_of_stock"].includes(status)||!ids.length)return NextResponse.json({error:"批次更新內容不正確。"},{status:400});return NextResponse.json({ok:true,updated:await bulkUpdateProductStatus(ids,status as "active"|"draft"|"out_of_stock",user.id)});}
export async function DELETE(request:Request){const user=await admin();if(!user)return NextResponse.json({error:"無管理權限。"},{status:403});let ids:number[]=[];try{const body=(await request.json()) as {ids?:unknown};ids=normalizeIds(body.ids);}catch{const id=Number(new URL(request.url).searchParams.get("id"));if(id)ids=[id];}if(!ids.length)return NextResponse.json({error:"缺少商品編號。"},{status:400});if(ids.length===1){await deleteProduct(ids[0],user.id);return NextResponse.json({ok:true,deleted:1});}return NextResponse.json({ok:true,deleted:await deleteProducts(ids,user.id)});}
