...
type AdminOrder = { id:number; number:string; customerName:string; customerPhone:string; total:number; status:string; paymentMethod:string; paymentStatus:string; shippingMethod:string; shippingCarrier:string|null; trackingNumber:string|null;cancellationReason:string|null; createdAt:string };
...
function OrdersPanel({ orders, update }: { orders: AdminOrder[]; update:(id:number,status:string,shippingCarrier?:string,trackingNumber?:string)=>void }) {
  const pending=orders.filter((order)=>["pending","confirmed","preparing"].includes(order.status)).length;
  return <section className="adminBody"><div className="statusStrip"><span>全部訂單 <b>{orders.length}</b></span><span>待處理 <b>{pending}</b></span><span>付款待確認 <b>{orders.filter((order)=>order.paymentStatus!=="paid"&&order.paymentStatus!=="cod").length}</b></span></div><section className="moduleCard"><div className="moduleHead"><div><p>ORDER MANAGEMENT</p><h2>訂單與配送處理</h2><span>建立訂單即保留庫存；取消才會自動回補。出貨時填物流資料，客戶可在會員頁查看。</span></div></div>{orders.length?<div className="orderList">{orders.map((order)=><OrderCard key={order.id} order={order} update={update}/>)}</div>:<div className="moduleEmpty"><h3>目前沒有訂單</h3><p>顧客完成結帳後，訂單會自動出現在這裡。</p></div>}</section></section>;
}

function OrderCard({order,update}:{order:AdminOrder;update:(id:number,status:string,shippingCarrier?:string,trackingNumber?:string)=>void}){
  ...
  useEffect(()=>{setStatus(order.status);setCarrier(order.shippingCarrier||"");setTracking(order.trackingNumber||"");},[order]);
  return <article className="fulfillmentCard"><div className="fulfillmentHead"><div><b>{order.number}</b><small>{new Date(order.createdAt).toLocaleString("zh-TW")} · {order.customerName}／{order.customerPhone}</small></div><strong>NT$ {order.total.toLocaleString()}</strong></div><div className="fulfillmentMeta"><span>{order.paymentMethod==="cod"?"貨到付款":order.paymentMethod}</span><span>{order.paymentStatus}</span><span>{order.shippingMethod}</span></div><div className="fulfillmentControls"><label>訂單狀態<select value={status} onChange={event=>setStatus(event.target.value)}>{[["pending","待確認"],["confirmed","已確認"],["preparing","備貨中"],["shipped","已出貨"],["completed","已完成"],["cancelled","取消訂單（回補庫存）"]].map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label>物流公司<select value={carrier} onChange={event=>setCarrier(event.target.value)}><option value="">尚未指定</option><option>綠界超商物流</option><option>黑貓宅急便</option><option>中華郵政</option></select></label><label>物流單號<input value={tracking} onChange={event=>setTracking(event.target.value)} placeholder={carrier==="綠界超商物流"?"綠界串接後自動帶入":"例如：1234567890"}/></label><button onClick={()=>update(order.id,status,carrier,tracking)}>{status==="shipped"?"儲存並標示出貨":"儲存變更"}</button></div></article>;
}
...
function ReturnsPanel({ returns, update, settings }: { returns: AdminReturn[]; update:(id:number,status:string,note:string,refundMethod?:string,refundAmount?:number,refundReference?:string)=>void; settings:StoreSettings }) {
  return <section className="adminBody"><section className="moduleCard"><div className="moduleHead"><div><p>RETURN MANAGEMENT</p><h2>退貨與退款處理</h2><span>流程：申請 → 同意退貨 → 收到退件（自動回補庫存）→ 完成退款。信用卡在綠界後台退刷後，再回來記錄結果。</span></div></div>{returns.length?<div className="returnList">{returns.map((item)=><ReturnCard key={item.id} item={item} update={update} settings={settings}/>)}</div>:<div className="moduleEmpty"><h3>目前沒有退貨申請</h3><p>顧客提出退貨申請後，將在這裡依流程處理與記錄。</p></div>}</section></section>;
}
...
function ReturnCard({item,update,settings}:{item:AdminReturn;update:(id:number,status:string,note:string,refundMethod?:string,refundAmount?:number,refundReference?:string)=>void;settings:StoreSettings}){
  ...
  useEffect(()=>{setStatus(item.status);setNote(item.resolutionNote||"");setMethod(item.refundMethod||"");setAmount(item.refundAmount?.toString()||"");setReference(item.refundReference||"");},[item]);
