...
function ProductModal({ editing, setEditing, save, upload, options }: { editing: Partial<CatalogProduct>; setEditing: (value: Partial<CatalogProduct> | null) => void; save: (e: React.FormEvent<HTMLFormElement>) => void; upload: (file:File) => Promise<string|undefined>; options:CatalogOptions }) {
  ...
  return <div className="modal"><form onSubmit={save}><div className="modalHead"><div><small>PRODUCT DATABASE</small><h2>{editing.id ? "編輯商品" : "新增商品"}</h2></div><button type="button" onClick={() => setEditing(null)}>×</button></div><div className="fields">
    ...
    <fieldset className="wide specificationField"><legend>商品規格</legend>{specifications.map((spec,index)=><div className="specRow" key={index}><input aria-label="規格名稱" placeholder="規格名稱，例如尺寸" value={spec.name} onChange={(event)=>updateSpec(index,"name",event.target.value)}/><input aria-label="規格選項" placeholder="選項用逗號分隔，例如 M, L, XL" value={spec.values.join(", ")} onChange={(event)=>updateSpec(index,"values",event.target.value)}/><button type="button" onClick={()=>setSpecifications((current)=>current.filter((_,i)=>i!==index))}>移除</button></div>)}<button className="addSpec" type="button" onClick={()=>setSpecifications((current)=>[...current,{name:"",values:[]}])}>＋ 新增規格</button><input type="hidden" name="specifications" value={JSON.stringify(specifications)}/><small>設定後會自動產生所有規格組合；最多建立 100 個組合。</small></fieldset>
    {variantRows.length>0&&<fieldset className="wide variantField"><legend>規格組合（SKU 變體）</legend><small>每個組合可設定自己的售價、庫存、商品編號及代表圖片；停用後客戶無法選購。</small><div className="variantTable"><div className="variantTableHead"><b>規格組合</b><b>商品編號</b><b>售價</b><b>庫存</b><b>圖片</b><b>啟用</b></div>{variantRows.map((variant,index)=>{const key=variantKey(variant.options,specifications);return <div className="variantRow" key={key}><strong>{specifications.map((group)=>variant.options[group.name]).join(" / ")}</strong><input aria-label={`${key} 商品編號`} placeholder={`變體 ${index+1}`} value={variant.sku} onChange={(event)=>updateVariant(key,"sku",event.target.value)}/><input aria-label={`${key} 售價`} type="number" min="0" value={variant.price} onChange={(event)=>updateVariant(key,"price",Number(event.target.value))}/><input aria-label={`${key} 庫存`} type="number" min="0" value={variant.stock} onChange={(event)=>updateVariant(key,"stock",Number(event.target.value))}/><select aria-label={`${key} 圖片`} value={variant.image||""} onChange={(event)=>updateVariant(key,"image",event.target.value)}><option value="">使用商品主圖</option>{images.map((url,imageIndex)=><option value={url} key={url}>圖片 {imageIndex+1}</option>)}</select><label className="variantToggle"><input type="checkbox" checked={variant.isActive} onChange={(event)=>updateVariant(key,"isActive",event.target.checked)}/><span>{variant.isActive?"是":"否"}</span></label></div>})}</div><input type="hidden" name="variants" value={JSON.stringify(variantRows)}/></fieldset>}
